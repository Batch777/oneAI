...
- `/home/steven/oneAI` 已具备完整 Git 历史，origin 为 `https://github.com/Batch777/oneAI.git`。GitHub CLI 授权已恢复，本轮原始提交历史已推送到 `codex/personal-assistant-foundation`；两端也保留了 Git bundle 迁移历史。
...
- 31 项发布管理器/Web Python 测试通过（Mac 和 Debian，含 commit 选择器拒绝测试）；26 项客户端测试通过（Mac，Linux 先跑 23 项既有测试，后跑 3 项更新提示测试）。
- 已连接真机 iPhone 17 Pro，开发者模式 enabled；用户随后要求配置签名，已完成 Personal Team 自动签名、Release 构建、签名校验与真机安装；用户在手机信任开发者后，真机启动复测成功。详见 `apps/apple/README.md`。
